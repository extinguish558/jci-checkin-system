...
const AdminPanel: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-5 md:space-y-6 pb-48">
      {/* 頁面標題 */}
          ...
          <div className="md:col-span-1 flex flex-col gap-4">
            ...
            >
              <div className="w-10 h-10 md:w-16 md:h-16 bg-blue-50 text-blue-500 rounded-xl md:rounded-[1.8rem] flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm shrink-0">
                <UserPlus size={20} className="md:w-8 md:h-8" />
              </div>
      ...
      {/* 分類導覽標籤 - 手機版縮小按鈕 Padding 與字體 */}
      <div className={`sticky top-11 md:top-0 z-40 -mx-4 md:-mx-8 px-4 md:px-8 py-3 transition-all duration-300 ${isSticky ? 'ios-blur bg-white/70 shadow-md border-b border-white/20' : ''}`}>
        <div className="flex bg-white/50 backdrop-blur-md p-1.5 md:p-2 rounded-[1.5rem] md:rounded-[2.2rem] overflow-x-auto gap-1 border border-white shadow-sm no-scrollbar">
            {groupedData.map(group => (
                <button 
                  key={group.key} 
                  onClick={() => setActiveTab(group.key)} 
                  className={`px-4 md:px-7 py-2.5 md:py-4 rounded-[1.2rem] md:rounded-[1.8rem] font-black text-[11px] md:text-sm whitespace-nowrap transition-all flex items-center gap-1.5 md:gap-2 ${activeTab === group.key ? 'bg-white text-slate-900 shadow-sm border border-gray-100' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  ...
                  {group.title.split(' ')[0]} 
                  <span className={`text-[9px] md:text-[11px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === group.key ? 'bg-slate-100 text-slate-600' : 'text-slate-300'}`}>{group.totalCount}</span>
                </button>
            ))}
        </div>
      </div>

      {/* 嘉賓名單列表 - 縮小單元格 Padding */}
                      ...
                      {group.list.map((g, idx) => (
                          <div key={g.id} className="px-5 md:px-8 py-4 md:py-7 flex items-center gap-3 md:gap-5 hover:bg-slate-50/50 transition-colors group">
                            ...
                            <div className="flex gap-1.5 md:gap-2.5 items-center">
                                {isAdmin && (
                                  <button onClick={() => setEditingGuest({...g})} className="hidden md:flex w-10 h-10 items-center justify-center bg-gray-50 text-gray-400 rounded-xl hover:bg-gray-100 hover:text-gray-600 transition-all opacity-0 group-hover:opacity-100">
                                    <Edit2 size={16}/>
                                  </button>
                                )}
            ...
            <form onSubmit={handleManualSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-none rounded-[1.2rem] p-4 font-black text-lg outline-none focus:ring-4 focus:ring-blue-500/10" 
                  placeholder="輸入姓名"
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 py-4 bg-blue-600 text-white font-black rounded-[1.2rem] shadow-lg active:scale-95 text-sm">確定新增</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-xs shadow-2xl flex flex-col items-center gap-6 border border-white/20">
            ...
            <form onSubmit={handleLoginSubmit} className="w-full space-y-5 text-center">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border-none rounded-[1.5rem] py-5 px-4 text-center text-4xl font-black outline-none tracking-widest" 
                autoFocus 
              />
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowLoginModal(false)} className="flex-1 py-4 font-black text-slate-300 text-xs">取消</button>
                <button type="submit" className="flex-1 py-4 bg-slate-900 text-white font-black rounded-[1.2rem] shadow-xl active:scale-95 text-xs">解鎖權限</button>
              </div>
            </form>
          </div>
        </div>
