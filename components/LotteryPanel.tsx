...
const LotteryPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (trigger.timestamp > lastHandledTriggerRef.current) {
      ...
      const selectedWinners = trigger.winnerIds
  ...
  const handleDrawClick = () => {
    ...
    armTimerRef.current = setTimeout(() => {
      setConfirmState('idle');
      drawWinners(drawCount);
    }, 1200);
  };

  const handleResetClick = async () => {
    if (!isAdmin) { setShowLoginModal(true); return; }
    if (window.confirm('確定重置今日所有得獎紀錄？此操作將同步初始化所有連線設備。')) {
      // 立即清空本地顯示
      setCurrentWinnerBatch([]);
      setAnimatingWinner(null);
      // 執行雲端重置
      ...
      >
        ...
        <header className="relative w-full p-6 md:p-8 z-30 border-b border-white/5 bg-gradient-to-b from-black/95 to-transparent flex items-center justify-between">
           <div className="flex items-center gap-6">
              ...
              <div className="text-left">
                 <h2 className="text-white font-black text-lg md:text-3xl tracking-tight leading-none italic drop-shadow(0 4px 12px rgba(0,0,0,0.8))">
                   {settings.eventName}
                 </h2>
           ...
           <div className="px-4 py-2 bg-white/5 rounded-full border border-white/10 flex items-center gap-2.5">
              ...
              <span className="text-[9px] md:text-base font-black text-white italic tracking-tighter tabular-nums opacity-60 uppercase">
                Pool Available: {currentPoolSize}
              </span>
           </div>
        </header>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                      ...
                      return (
                        <div key={`${g.id}-${idx}`} className="flex flex-col items-center justify-center text-center px-4" style={{ height: `${ITEM_HEIGHT}px` }}>
                          <span className={`font-black italic transition-all duration-300 whitespace-nowrap leading-none tracking-tighter ${
                            isWinner 
                              ? `text-white text-3xl md:text-6xl drop-shadow(0 0 30px rgba(255,255,255,0.6)) ${isFlashing ? 'animate-winner-flash' : 'scale-110'}` 
          ...
          `}>
             <div className="flex items-center gap-2 mb-4 opacity-40">
                ...
                <span className="text-[10px] font-black text-white tracking-[0.4em] uppercase italic">Quantum Live Session Sync</span>
             </div>
             ...
             <div className="h-full flex flex-col gap-2 pb-24 overflow-y-auto no-scrollbar">
                {currentWinnerBatch.length > 0 ? (
                  currentWinnerBatch.map((w, idx) => (
                    <div key={`${w.id}-${idx}`} className="flex items-center p-3 md:p-5 bg-white/[0.05] border border-white/10 rounded-xl md:rounded-[1.5rem] animate-in slide-in-from-right-10 fade-in duration-700 shadow-2xl group hover:bg-white/[0.08] transition-all">
                      <div className="w-8 h-8 md:w-14 md:h-14 bg-blue-600 rounded-lg md:rounded-2xl flex items-center justify-center font-black italic text-white text-[10px] md:text-xl shrink-0 mr-4 md:mr-6 shadow(0 0 15px rgba(37,99,235,0.4))">
                        #{idx+1}
                      </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-5">
                     ...
                     <p className="font-black tracking-[1.2em] text-[8px] uppercase mt-8 italic">Awaiting Synchronized Trigger</p>
                  </div>
                )}
                ...
                {isAnimating && currentWinnerBatch.length < drawCount && (
                  <div className="p-8 md:p-14 border border-dashed border-white/20 rounded-[2rem] flex items-center justify-center gap-6 animate-pulse bg-white/[0.02] mt-4">
                     ...
                     <span className="text-xs md:text-2xl font-black text-white/20 uppercase tracking-[0.5em] italic">Synchronizing Animation...</span>
                  </div>
                )}
             </div>
          </div>
        ...
        {confirmState === 'armed' && (
           <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/98 backdrop-blur-3xl animate-in fade-in duration-500 text-center">
              ...
              <h1 className="text-lg md:text-4xl font-black text-white tracking-tighter italic uppercase">BROADCASTING COMMAND: R{currentActiveRound}</h1>
              <p className="text-[8px] md:text-xs text-white/30 font-black tracking-[0.8em] mt-6">QUANTUM SYNC ENGAGED</p>
           </div>
        )}
      </div>
      ...
      <div className="mt-8 md:mt-12 w-full max-w-4xl space-y-6 z-20">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-1 w-full relative group">
                ...
                >
                  {[1, 2, 3, 4, 5].map(n => <option key={n} value={n} className="bg-slate-950">Broadcast Draw x{n} Guests</option>)}
                </select>
        ...
        <div className="relative bg-[#15171e] rounded-[2rem] md:rounded-[3rem] border border-white/10 shadow-3xl overflow-hidden min-h-[450px]">
           ...
           <div className="p-6 md:p-10 flex items-center justify-between border-b border-white/5 bg-black/25 relative z-10">
              <div className="flex items-center gap-5">
                 ...
                 <div>
                    <h3 className="text-white font-black text-xl md:text-3xl italic tracking-tighter">Round {currentActiveRound} Master Archives</h3>
                    <p className="text-slate-500 font-bold text-[8px] md:text-xs uppercase tracking-[0.4em] mt-2 italic">此輪次同步之得獎紀錄</p>
                 </div>
              </div>
              <div className="text-right">
                 <span className="text-blue-500 font-black text-2xl md:text-5xl tabular-nums italic drop-shadow(0 0 15px rgba(59,130,246,0.3))">
                   {(historyMap[currentActiveRound] || []).length}
                 </span>
                 <p className="text-slate-600 font-bold text-[8px] md:text-[10px] uppercase tracking-widest mt-1">Live Records</p>
              </div>
           </div>
           ...
           <div className="p-4 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 relative z-10">
              ...
              {(historyMap[currentActiveRound] || []).length === 0 && (
                <div className="col-span-full py-32 flex flex-col items-center justify-center opacity-10">
                   ...
                   <p className="text-base font-black tracking-[1.5em] text-center uppercase mt-10 italic">Archive Initialized: Round {currentActiveRound}</p>
                </div>
              )}
           </div>
        </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 ios-blur bg-black/98 z-[500] flex items-center justify-center p-6">
          <div className="bg-[#0b0c10] border border-white/10 rounded-[3rem] p-10 md:p-16 max-sm w-full shadow-3xl flex flex-col items-center gap-8 text-center animate-in zoom-in-95 duration-500">
            <h3 className="text-base md:text-2xl font-black text-white tracking-[0.5em] italic uppercase">Master Control Key</h3>
            <form onSubmit={handleLoginSubmit} className="w-full space-y-10">
              ...
              <div className="flex gap-6">
                ...
                <button type="submit" className="flex-1 py-6 bg-blue-600 text-white font-black rounded-2xl text-[10px] md:text-xs uppercase tracking-[0.4em] active:scale-95">Verify</button>
              </div>
            </form>
          </div>
        </div>
