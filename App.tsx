...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-[#F2F2F7] overflow-hidden">
      ...
      {/* 全域頂部狀態列 (Global Status Bar) */}
      <header className="ios-blur bg-white/70 border-b border-gray-200/50 px-6 py-3 sticky top-0 z-[150] flex justify-between items-center h-14 shrink-0">
        <div className="flex items-center gap-2 overflow-hidden">
          <div className="w-1.5 h-6 bg-[#007AFF] rounded-full hidden md:block" />
          <span className="text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest truncate max-w-[150px] md:max-w-none">
            {settings.eventName || "活動系統"}
          </span>
        </div>
        
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 bg-black/5 px-3 py-1 rounded-full border border-black/5">
        ...
        <div className="max-w-2xl mx-auto pb-safe">
          <nav className="ios-blur bg-black/85 text-white shadow-[0_25px_60px_rgba(0,0,0,0.4)] rounded-[2.5rem] h-20 md:h-22 flex justify-around items-center px-2 md:px-4 border border-white/10 ring-1 ring-white/5">
            ...
            <button onClick={() => setActiveTab('mcflow')} className={`flex flex-col items-center gap-1.5 transition-all flex-1 ${activeTab === 'mcflow' ? 'text-[#007AFF]' : 'text-gray-400'}`}>
              {/* Fix: Corrected typo in drop-shadow utility class */}
